...
def check_database_cleanup():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if expired_count > 0:
                logger.warning(f"Database cleanup needed: {expired_count} expired codes found")
                return False
            else:
                logger.info("Database cleanup working correctly - no expired codes found")
                return True
    except Exception as e:
...
def check_rate_limiting():
    ...
    try:
        ...
        logger.info(f"  - Verification cooldown: 30 seconds")
        logger.info(f"  - Max verification attempts: 3")
        logger.info(f"  - Max resend attempts: 3")
...
def run_cleanup_test():
    ...
    try:
        ...
        logger.info(f"Cleanup process removed {cleaned_count} expired verification codes")
        ...
        with get_db_session() as session:
            ...
            if record and record[0] is None and record[1] is None and record[2] is False:
                logger.info("Cleanup test passed - verification fields were reset")
                return True
            else:
                logger.error(f"Cleanup test failed - verification fields not reset properly: {record}")
                return False
