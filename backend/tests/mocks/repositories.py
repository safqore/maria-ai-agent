...
from backend.app.errors import ServerError, ResourceNotFoundError
            ...
            with get_db_session() as session:
                ...
                if isinstance(session_uuid, str):
                    try:
                        ...
                        print(f"Warning: Invalid UUID format in get_by_uuid: {session_uuid}")
                ...
                result = session.query(UserSession).filter(UserSession.uuid == session_uuid).first()
                
                # If that fails, try alternative approaches
                if result is None:
                    # Try with string representation as fallback
                    try:
                        result = session.query(UserSession).filter(UserSession.uuid == str(session_uuid)).first()
                    except Exception:
                        pass
    ...
    def exists(self, session_uuid: str) -> bool:
        ...
        try:
            ...
            with get_db_session() as db_session:
                result = db_session.query(UserSession).filter(UserSession.uuid == uuid_object).first()
            ...
            with get_db_session() as session:
                ...
                if isinstance(kwargs['uuid'], str):
                    try:
                        ...
                        print(f"Warning: Invalid UUID format in create: {kwargs['uuid']}")
                ...
                return UserSession(uuid=uuid_value, **{k: v for k, v in kwargs.items() if k != 'uuid'})
        except SQLAlchemyError as e:
    ...
    def update(self, session_uuid: str, **kwargs) -> Optional[UserSession]:
        ...
        try:
            with get_db_session() as session:
                ...
                user_session = session.query(UserSession).filter(UserSession.uuid == session_uuid).first()
                if not user_session:
                    return None
