...
class TestDatabasePerformance:
    ...
    def test_repository_create_performance(self, setup_test_data):
        ...
        assert avg_time < 0.1, f"Average create time {avg_time:.3f}s exceeds 100ms threshold"
        assert max_time < 0.2, f"Max create time {max_time:.3f}s exceeds 200ms threshold"
        
        print(f"Repository create performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_repository_get_by_uuid_performance(self, setup_test_data):
        ...
        assert avg_time < 0.05, f"Average get_by_uuid time {avg_time:.3f}s exceeds 50ms threshold"
        assert max_time < 0.1, f"Max get_by_uuid time {max_time:.3f}s exceeds 100ms threshold"
        
        print(f"Repository get_by_uuid performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_email_lookup_performance(self, setup_test_data):
        ...
        assert avg_time < 0.05, f"Average email lookup time {avg_time:.3f}s exceeds 50ms threshold"
        assert max_time < 0.1, f"Max email lookup time {max_time:.3f}s exceeds 100ms threshold"
    ...
    def test_verification_status_query_performance(self, setup_test_data):
        ...
        with get_db_session() as session:
            for verified in [True, False]:
                with self.performance_timer():
                    results = session.query(UserSession).filter(
                        UserSession.is_email_verified == verified,
                        UserSession.completed_at.is_(None)
                    ).all()
                execution_times.append(self.last_execution_time)
                assert len(results) > 0, f"Should find sessions with is_email_verified={verified}"
        
        avg_time = statistics.mean(execution_times)
        max_time = max(execution_times)
        
        # Assert performance expectations for composite index
        assert avg_time < 0.1, f"Average verification query time {avg_time:.3f}s exceeds 100ms threshold"
        assert max_time < 0.2, f"Max verification query time {max_time:.3f}s exceeds 200ms threshold"
        
        print(f"Verification status query performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_bulk_operations_performance(self, setup_test_data):
        ...
        assert avg_time < 0.2, f"Average bulk operation time {avg_time:.3f}s exceeds 200ms threshold"
        assert max_time < 0.5, f"Max bulk operation time {max_time:.3f}s exceeds 500ms threshold"
    ...
    def test_transaction_context_performance(self, setup_test_data):
        ...
        for i in range(10):
            with self.performance_timer():
                with TransactionContext() as session:
                    ...
                    # Query the session
                    retrieved = session.query(UserSession).filter(
        ...
        assert avg_time < 0.1, f"Average transaction time {avg_time:.3f}s exceeds 100ms threshold"
        assert max_time < 0.2, f"Max transaction time {max_time:.3f}s exceeds 200ms threshold"
        
        print(f"Transaction context performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_lazy_loading_verification(self, setup_test_data):
        ...
        with self.performance_timer():
            ...
            attrs = [
                session.uuid, session.name, session.email, session.created_at,
                session.updated_at, session.completed_at, session.ip_address,
                session.consent_user_data, session.is_email_verified,
                session.verification_code, session.verification_attempts
            ]
        
        # Should be very fast since all data is already loaded
        assert self.last_execution_time < 0.001, f"Attribute access took {self.last_execution_time:.3f}s"
        
        print(f"Lazy loading verification: attribute access time={self.last_execution_time:.6f}s")
    ...
    def test_connection_pooling_performance(self):
        ...
        assert avg_time < 0.05, f"Average connection time {avg_time:.3f}s suggests pooling issues"
        assert max_time < 0.1, f"Max connection time {max_time:.3f}s suggests pooling issues"
        
        print(f"Connection pooling performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_concurrent_access_performance(self, setup_test_data):
        ...
        assert avg_time < 1.0, f"Average concurrent operation time {avg_time:.3f}s exceeds 1s threshold"
        assert max_time < 2.0, f"Max concurrent operation time {max_time:.3f}s exceeds 2s threshold"
        
        print(f"Concurrent access performance: avg={avg_time:.3f}s, max={max_time:.3f}s")
    ...
    def test_index_usage_verification(self, setup_test_data):
        ...
        for operation_name, operation in operations:
            ...
            assert self.last_execution_time < 0.1, f"{operation_name} took {self.last_execution_time:.3f}s"
