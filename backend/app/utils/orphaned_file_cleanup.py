...
Deletes S3 folders under uploads/{uuid}/ that are older than 30 minutes and have no matching session in the user_sessions table.
...
def list_s3_folders_older_than(s3, bucket, prefix, min_age_minutes):
    """Yield (uuid, last_modified) for each folder under prefix older than min_age_minutes."""
    ...
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix, Delimiter='/'):
        for cp in page.get('CommonPrefixes', []):
            folder = cp['Prefix']
            uuid = folder[len(prefix):-1]  # Remove prefix and trailing slash
...
def list_base_files_older_than(s3, bucket, prefix, min_age_minutes):
    """Yield (key, last_modified, age_minutes) for files directly under prefix (not in subfolders) older than min_age_minutes."""
    now = datetime.now(timezone.utc)
    paginator = s3.get_paginator('list_objects_v2')
    for page in paginator.paginate(Bucket=bucket, Prefix=prefix, Delimiter='/'):
        for obj in page.get('Contents', []):
            key = obj['Key']
            # Only consider files directly under uploads/ (no further slashes after prefix)
...
def main():
    ...
    for uuid, folder, oldest, age_minutes in list_s3_folders_older_than(s3, S3_BUCKET, S3_UPLOAD_PREFIX, AGE_THRESHOLD_MINUTES):
        valid_uuids = get_valid_session_uuids()
        if uuid in valid_uuids:
            logger.info(f"Skipping {folder}: UUID {uuid} is now valid.")
            continue
        msg = f"Orphaned folder {folder} (UUID={uuid}, age={age_minutes:.1f} min, oldest={oldest})"
        if DRY_RUN:
    ...
    for key, lm, age_minutes in list_base_files_older_than(s3, S3_BUCKET, S3_UPLOAD_PREFIX, AGE_THRESHOLD_MINUTES):
