...
class VerificationService:
    ...
    def send_verification_code(self, session_id: str, email: str) -> Dict[str, Any]:
        ...
        with TransactionContext():
            try:
                # Get user session
                user_session = self.email_verification_repository.get_by_session_id(session_id)
                if not user_session:
                    return {
                        'status': 'error',
                ...
                # Send email
                if not self.email_service.send_verification_email(email, code, session_id):
                    return {
                        'status': 'error',
    ...
    def verify_code(self, session_id: str, code: str) -> Dict[str, Any]:
        ...
        with TransactionContext():
            try:
                ...
                if user_session.verification_attempts >= user_session.max_verification_attempts:
                    return {
                        'status': 'error',
                ...
                self.email_verification_repository.increment_verification_attempts(session_id)
                ...
                if not self.email_verification_repository.mark_email_verified(session_id):
                    return {
                        'status': 'error',
    ...
    def cleanup_expired_verifications(self, hours: int = 24) -> int:
        ...
        """
        try:
            count = self.email_verification_repository.cleanup_expired_verifications(hours)
